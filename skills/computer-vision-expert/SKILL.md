...
### Example 1: Real-Time Detection with YOLO26

```python
from ultralytics import YOLO

# Load the SOTA 2026 model
model = YOLO('yolo26n.pt')  # Nano model for edge deployment

# Inference without NMS post-processing overhead
...
for r in results:
    ...
    for box in boxes:
        ...
        cls = int(box.cls[0])
        print(f"Detected {model.names[cls]} at [{x1}, {y1}, {x2}, {y2}]")
```

### Example 2: Text-Guided Segmentation with SAM 3
...
# Text prompt for zero-shot segmentation
masks = predictor.predict(
    image,
    text_prompt="the red emergency stop button"
)
